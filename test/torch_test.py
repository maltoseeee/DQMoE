...
def test_dynamic_quantization_moe(
    ...
    for (
        ...
        def fn():
            bench_fn(a_, b_, b1_, d_, m_indices, workspace, quant_value)
        ...
        if t == 0:
            ...
            continue
        print(
            f"> Grouped Gemm metric for ({num_groups=},num_tokens={num_tokens:5}, hidden_size={hidden_size:6}, inter_size={inter_size:5}, {quant_type}, layout=kmajor)".center(
                100, "="
            )
        )
        print(
            ...
            f"{count_bytes(a_, b_, b1_, d_, m_indices) / (2 * t) / 1e9:4.0f} GB/s | "
        )
